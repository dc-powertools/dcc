...
data a Feature `install.sh`, Dockerfile layer, or official `build` source places
at a declared state path would be silently masked by an empty directory or file.
...
are available to Feature `install.sh` scripts during `docker build` and remain
set at runtime. Only the fixed container-side variables
`${containerWorkspaceFolder}` and `${containerCacheFolder}` may appear in
`containerEnv` values.
...
Each Feature `install.sh` runs as `root`, matching the containers.dev Feature
spec. `dcc` exports `_REMOTE_USER`, `_CONTAINER_USER`, `_REMOTE_USER_HOME`, and
`_CONTAINER_USER_HOME` for scripts that need to switch to `containerUser`.
...
rebuild. Pass `--update` to re-resolve upstream inputs and pass `--pull` to
`docker build`. Pass `--refresh-only` to skip image rebuild and rerun only
...
`postCreateCommand` in order, with Feature hooks before project hooks for each
phase and state mounts attached.
...
Run `dcc build` after changing Features.
...
`dcc run` runs a named command from `customizations.dcc.commands` or Feature
command metadata. With no command argument, it lists available commands. Project
...
For in-container hooks, Feature-contributed hooks run first in Feature
installation order, followed by the `devcontainer.json` hook of that type. A
non-zero exit aborts the current operation and skips subsequent hooks.
