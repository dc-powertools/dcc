...
pub(crate) async fn forward_ports(container: &str, ports: &[u16]) -> anyhow::Result<Forwarding> {
    let listeners = bind_forwarding_listeners(ports, |listener, port| (listener, port)).await?;
...
async fn bind_forwarding_listeners<OwnedListener>(
    ports: &[u16],
    mut own_listener: impl FnMut(TcpListener, u16) -> OwnedListener,
) -> anyhow::Result<Vec<OwnedListener>> {
    ...
    for &port in ports {
        ...
        listeners.push(own_listener(v4, port));
        ...
        if let Some(v6) = optional_ipv6_listener(v6, &v6_address, port) {
            listeners.push(own_listener(v6, port));
        }
    }
    Ok(listeners)
}
...
"#,
    ...
    async fn later_bind_collision_releases_every_listener_without_spawning_tasks() {
        ...
        let mut earlier_listeners = Vec::new();

        let error = match bind_forwarding_listeners(&[0, collision_port], |listener, port| {
            let listener = Arc::new(listener);
            earlier_listeners.push(Arc::downgrade(&listener));
            (listener, port)
        })
        .await
        {
        ...
        assert!(!earlier_listeners.is_empty());

        for listener in earlier_listeners {
            assert!(
                listener.upgrade().is_none(),
                "earlier listener remained owned after later collision"
            );
        }
    }
