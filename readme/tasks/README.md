...
| T-0005 | Implement devcontainer schema-compatible config parsing and merge behavior under `customizations.dcc`, including legacy `extends`/`scripts` warnings. | Parent T-0004 / r1 | Active | None | Initiative / High | None | Implement `customizations.dcc` config model, merge tests, and compatibility warnings. | `readme/tasks/0005-config-schema-brief.md` | None |
| T-0006 | Implement validated `customizations.dcc.state` path model and profile-local state cache mount planning. | Parent T-0004 / r1 | Pending | T-0005 | Initiative / High | None | Start after T-0005 is Done; design state path normalization and cache layout tests. | None | None |
