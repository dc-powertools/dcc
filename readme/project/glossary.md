...
| `extends` | Local devcontainer config inheritance property resolved relative to the child config file. | Include, import | `README.md`; `readme/project/architecture.md`; `src/config/resolve.rs` | 2026-07-14 |
| `containerEnv` | Build-time environment baked into the image as Docker `ENV` directives. | Runtime env | `README.md`; `readme/project/architecture.md` | 2026-07-14 |
| `remoteEnv` | Runtime environment passed to `docker run` as `-e` flags and re-evaluated on each run. | Image env | `README.md`; `readme/project/architecture.md` | 2026-07-14 |
