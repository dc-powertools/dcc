...
| --- | --- | --- |
| `.meta/README.md` | Always-read pointer to the task catalog and primary task, global approvals, recent outcomes, and maintenance cursor | Root Orchestrator, every session and close |
| `.meta/project/brief.md` | Product purpose, users, outcomes, and constraints | Agents update from product evidence |
| `.meta/project/context.md` | Concise stack, technical conventions, and conflict-prone implementation rules | Agents update from code and decisions |
| `.meta/project/standards.md` | Project-specific rules and the sole canonical command catalog | Agents update only from observed practice and executed commands |
| `.meta/project/assumptions.md` | Open uncertainty, confidence, impact, and validation | Agent that introduces or resolves the assumption |
| `.meta/project/glossary.md` | Canonical domain terms and deprecated synonyms | Agent ingesting or changing domain language |
| `.meta/project/source-map.md` | Important sources, trust, ownership, and freshness | Agent relying on the source |
| `.meta/project/automation-backlog.md` | Evidence-backed candidates for removing repeated manual work | Agent observing the candidate |
| `.meta/project/agents.md` | Durable project-specific roles or agent rules justified by repeated use | Root Orchestrator |
| `.meta/decisions/` | Append-only significant choices and consequences | Decision owner or Root Orchestrator |
| `.meta/tasks/README.md` | Stable task IDs and revisions, authority provenance, global scheduling pause, outcomes, lifecycle, dependencies, selected route/risk, task approvals or blockers, next actions, detail links, and results | Root Orchestrator, sole writer |
| `.meta/tasks/NNNN-*-brief.md` | Task-specific scope, acceptance, route/risk rationale, and amendments when the catalog row is insufficient | Root Orchestrator or assigned analyst |
| `.meta/tasks/NNNN-*-notes.md` | Execution checkpoints for long-running, risky, paused, or parallel work | Root Orchestrator |
| `.meta/quality/` | Durable readiness, verification, review, and completion evidence | Root Orchestrator or QA owner |
| `.meta/threat-models/` | Security and trust-boundary analysis | Security or risk owner |
| `.meta/incidents/` | Blameless incident and near-miss learning | Incident owner |
| `.meta/learning/retrospectives.md` | Searchable cross-session correction and process-learning signals | Agent observing the signal |
| `.meta/learning/framework-changelog.md` | Auditable framework edits, pilots, and sunset triggers | Agent changing the framework |

`.meta/README.md` and `.meta/tasks/README.md` are mandatory after onboarding because
...
- One decision per `.meta/decisions/NNNN-short-title.md`.
...
| `.meta/README.md` | 80 lines, hard | Move detail to task notes or decisions; retain only current pointers | Root Orchestrator |
...
| Assumptions | 120 lines | Archive closed rows to `.meta/archive/` | Root Orchestrator |
...
Archive under `.meta/archive/` with a date or sequence in the filename. Do not create an
