...
| On Linux with a non-root `containerUser`, the uid/gid is remapped to the host's before container creation. | `plan_uid_remap` unit tests + Dockerfile generation tests + dry-run report. | T-0045 replaced the host-dependent Unix test with `simulated_linux_remaps_non_root_named_user`; `dockerfile_emits_remap_block_when_planned` asserts the `ARG`/`RUN` shape and ordering after user creation; local `dcc build --dry-run --format json` reports `updateRemoteUserUID remap planned: user \`dev\` -> uid 1000 gid 1000`. Live Docker execution delegated to CI. | Pass |
| The remap no-ops when: root, numeric, already-matching, uid collision, non-Linux, disabled. | `plan_uid_remap` unit tests. | `plan_skips_when_disabled`, `_skips_root_user`, `_skips_numeric_user`, `_skips_when_host_ids_unavailable`, `fast_path_config_implies_no_remap`, and T-0045's simulated macOS/Windows branch all pass. The `remap_run_script` carries the inline collision/gid-collision no-op echoes (`remap_block_includes_collision_noop_echo`). | Pass |
